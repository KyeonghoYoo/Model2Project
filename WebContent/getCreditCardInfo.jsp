...
<%
	...
	CreditCardInfoVO creditCardInfo = (CreditCardInfoVO) session.getAttribute("creditCardInfo");
	StringTokenizer tokens = new StringTokenizer(creditCardInfo.getCardNum());
	String num1 = tokens.nextToken("-");
	String num2 = tokens.nextToken("-");
	String num3 = tokens.nextToken("-");
	String num4 = tokens.nextToken("-");
	StringTokenizer tokens2 = new StringTokenizer(creditCardInfo.getExpirationDate());
	...
	<div id="page-wrapper">
		...
		<div id="content">
			<section id="main-section">
				...
				<form action="updateCreditCardInfo.do" method="post">
					<input type="hidden" name="exCardNum" value="<%=creditCardInfo.getCardNum() %>"/>
					<table border="1" cellpadding="0" cellspacing="0">
						<tr>
							<td bgcolor="#CCCCCC">카드번호</td>
							<td>
							<input type="text" name="num1" maxlength="4" value="<%=num1 %>"/>
							<input type="text" name="num2" maxlength="4" value="<%=num2 %>"/>
							<input type="text" name="num3" maxlength="4" value="<%=num3 %>"/>
							<input type="text" name="num4" maxlength="4" value="<%=num4 %>"/></td>
						</tr>
						<tr>
							<td bgcolor="#CCCCCC">유효기간</td>
							<td>
							<input type="text" name="month" placeholder="MM" maxlength="2" value="<%=month %>"/>
							<input type="text" name="year" placeholder="YY" maxlength="2" value="<%=year %>"/></td>
						</tr>
						<tr>
							<td bgcolor="#CCCCCC">카드종류</td>
							<td><select name="cardType">
									<%if(creditCardInfo.getCardType().equals("신용카드")){ %>
										<option value="체크카드">체크카드</option>
										<option value="신용카드" selected="selected">신용카드</option>
									<% } else {%>
										<option value="체크카드" selected="selected">체크카드</option>
