...
function btn_click(num) {
	if(num == 1){
		document.orderForm.action = "getAddressInfoList.do";
	} else if(num == 2){
		document.orderForm.action = "getCreditCardInfoList.do";
	} else if(num == 3){
		document.orderForm.action = "getCustomerOrderList.do";
	}
			...
			<section id="main-section">
				...
				<input name="customerId" type="hidden" value="<%=customer.getCustomerId()%>"/>
					<table border="1" cellpadding="0" cellspacing="0">
						<tr>
							<td bgcolor="#CCCCCC">회원아이디</td>
							<td><%=customer.getCustomerId() %></td>
						</tr>
						<tr>
							<td bgcolor="#CCCCCC">회원이름</td>
							<td><%=customer.getName() %></td>
						</tr>
						<tr>
							<td colspan="2" align="center">
								<input type="button" value="배송지관리" onclick="btn_click(1)"/>   
								<input type="button" value="카드정보관리" onclick="btn_click(2)" />
								<input type="button" value="주문내역조회" onclick="btn_click(3)" />
							</td>
						</tr>
					</table>
				</form>
